...
# Define um tipo de função de manipulador de erro C
...
def no_alsa_err():
    ...
    try:
        ...
        # Se a libasound não for encontrada (ex: em Windows/macOS), apenas continua
        yield
...
def run_voice_cli():
    ...
    # Usa o novo gerenciador de contexto para silenciar o ALSA durante a inicialização
    with no_alsa_err():
        stt_instance = STT()
        tts_instance = TTS()
    ...
    while True:
        try:
            print("Você: (Ouvindo...)")
            # E também durante cada chamada de escuta
            with no_alsa_err():
            ...
            if prompt.startswith("Não entendi") or prompt.startswith("Erro no serviço"):
